...
async function starts() {
	...
	selfb.on('chat-update', async (mek) => {
		try {
            ...
            const ramadhan = await fetchJson(`https://xinzbot-api.herokuapp.com/api/hitungmundur?apikey=XinzBot&tanggal=12&bulan=4`)
            const hahh = await fetchJson(`https://xinzbot-api.herokuapp.com/api/ucapan?apikey=XinzBot&timeZone=Asia/Jakarta`) 
...
/********** END FUNCTION ***************/
			...
			switch(command) {
				case 'help':
				case 'menu':
				wuuw = `┃(${time}) Selamat ${hahh.result}\n┃${tanggal} ${day} ${bulan} ${teh}\n┃Ramadhan : ${ramadhan.result}`
...
┗━━━━《 SelfBot Recode 》━━━━`, MessageType.text, {quoted: freply}, {contextInfo: { forwardingScore: 508, isForwarded: true }})
			...
			selfb.sendMessage(from, `Status:${public}`, text)
                ...
                const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model, batterylevel } = selfb.user.phone
                runtime = process.uptime()
                teskny = `┏━━━━《 MY-stat 》━━━━
